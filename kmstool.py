...
working_dir = "/tmp/kmstool/"

def get_default_region():
    home = expanduser("~")
    aws_creds = ConfigParser()
    aws_creds.read('%s/.aws/config' % home)
    return { 'region': aws_creds.get('default', 'region') }
...
# Recursively removes given path.
def rm_rf(path_to_remove):
...
def main():
    ...
    enc_file = os.path.join(working_dir, 'file.enc')
    cipher_file = os.path.join(working_dir, 'key.enc')
