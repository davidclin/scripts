...
# Variables
RESOURCE=ebs,ebs-snapshot,security-group,s3,ami,dynamodb-table,dynamodb-stream,d
ynamodb-backup,elasticsearch,elb,eni,lambda,lambda-layer,rds,s3,sqs,sns,cfn
REGION="--region us-east-1"
CUSTODIAN_POLICY="/home/ubuntu/cloudcustodian/policies/iam-user-tagged-resources
-audit.yml"
...
# User input
read -p "Enter AWS username that is getting offboarded: " aws_username
read -p "Enter your GHE username: " ghe_username
read -s -p "Enter your GHE Oauth Token: " ghe_password
...
custodian report -s $CUSTODIAN_OUTPUT_DIRECTORY -t ec2 $CUSTODIAN_POLICY --field
 tag:Owner=tag:Owner --format grid >> $CUSTODIAN_REPORT
...
do
    ...
    custodian report -s $CUSTODIAN_OUTPUT_DIRECTORY -t $i $CUSTODIAN_POLICY --fi
eld tag:Owner=tag:Owner --format grid >> $CUSTODIAN_REPORT
    echo ' ' >> $CUSTODIAN_REPORT
done
